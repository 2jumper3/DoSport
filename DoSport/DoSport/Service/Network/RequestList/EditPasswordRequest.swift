...
protocol EditPasswordRequestFactory {
    func editPassword(token: String, newPassword: String, newPasswordConfirm: String, oldPassword:String, completionHandler: @escaping (AFDataResponse<String>) -> Void)
}
...
extension EditPasswordRequest: EditPasswordRequestFactory {
    func editPassword(token: String, newPassword: String, newPasswordConfirm: String,
                  oldPassword:String, completionHandler: @escaping (AFDataResponse<String>) -> Void) {
        let header = HTTPHeader(name: "Authorization", value: token)
        let headers = HTTPHeaders([header])
        let requestModel = EditPassword(headers: headers, baseUrl: baseUrl, newPassword: newPassword,
                                        newPasswordConfirm: newPasswordConfirm, oldPassword: oldPassword)
